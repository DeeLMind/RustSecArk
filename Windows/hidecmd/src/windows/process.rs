...
use windows::{
    ...
    Win32::{
        ...
        System::Threading::{
            CreateProcessW, GetExitCodeProcess, TerminateProcess, WaitForSingleObject, INFINITE, PROCESS_CREATION_FLAGS, PROCESS_INFORMATION, STARTF_USESHOWWINDOW, STARTUPINFOW
        },
...
impl Command {
    ...
    ///     .expect("notepad failed to start");
    /// ```
    pub fn spawn(&mut self,no_gui:bool) -> Result<Child> {
        Child::new(
            &self.command,
    ...
    /// ```
    pub fn status(&mut self,no_gui:bool) -> Result<ExitStatus> {
        self.spawn(no_gui)?.wait()
    }
}
...
pub struct Child {
    ...
    output_write_handle: HANDLE,
    error_read_handle: HANDLE,
    error_write_handle: HANDLE,
}
...
impl Child {
    ...
    fn new(
        ...
        if no_gui {
            startup_information.dwFlags |= STARTF_USESHOWWINDOW;  // Set to hide the window
            startup_information.wShowWindow = 0;  // Hides the window
        }
        ...
        let mut output_read_handle = HANDLE(0);
        let mut output_write_handle = HANDLE(0);
        let mut error_read_handle = HANDLE(0);
        let mut error_write_handle = HANDLE(0);

        if !unsafe { CreatePipe(&mut output_read_handle, &mut output_write_handle, &mut sa, 0) } {
            return Err(Error::CreationFailed(unsafe { GetLastError().0 }));
        }

        if !unsafe { CreatePipe(&mut error_read_handle, &mut error_write_handle, &mut sa, 0) } {
            return Err(Error::CreationFailed(unsafe { GetLastError().0 }));
        }
    ...
    // Function to get the command line result (stdout + stderr)
    pub fn get_command_line_result(&self) -> Result<String> {
        let mut output = String::new();
        
        // Read from the output pipe
        let mut buffer = [0u8; 4096]; // Read up to 4096 bytes at a time
        let mut bytes_read = 0;
        
        unsafe {
            while ReadFile(self.output_read_handle, &mut buffer, &mut bytes_read).is_ok() && bytes_read > 0 {
                output.push_str(&String::from_utf8_lossy(&buffer[..bytes_read]));
            }
        }

        // Read from the error pipe (stderr)
        let mut error_output = String::new();
        
        unsafe {
            while ReadFile(self.error_read_handle, &mut buffer, &mut bytes_read).is_ok() && bytes_read > 0 {
                error_output.push_str(&String::from_utf8_lossy(&buffer[..bytes_read]));
            }
        }

        // Combine stdout and stderr results
